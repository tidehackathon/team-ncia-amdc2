	...
	this.type.subscribe(function(newValue)
	{
		...
		if((newValue in datasourcePlugins) && _.isFunction(datasourcePlugins[newValue].newInstance))
		{
			...
			function finishLoad()
			{
				...
				settings.selfName = selfName;
				datasourceType.newInstance(settings, function(datasourceInstance)
				{
...
function FreeboardUI()
{
	...
	{
		var available_width = $("#board-content").width();
		return MIN_COLUMNS;//stupid alex commented this!!!!Math.floor(available_width / COLUMN_WIDTH);
	}
					...
					pluginEditor.createPluginEditor(title, types, instanceType, settings, function(newSettings)
					{
						if(options.operation == 'add')
						{
							...
							else if(options.type == 'widget')
							{
								...
								//Alex added; Force redraw of board to avoid pane grid placement bug
								freeboard.loadDashboard(freeboard.serialize(), function(){});
							}
						}
		...
		update: function(element, valueAccessor, allBindingsAccessor, viewModel, bindingContext)
		{
			...
			if(theFreeboardModel.panes.indexOf(viewModel) == -1)
			{
				try {
					...
					console.log('Stupidly silencing error here!!!!!!!!!!!!!!');
				}
			}
			try {
				freeboardUI.updatePane(element, viewModel);
			} catch(e) {
				console.log('Stupidly silencing error here too!!!!!!!!!!!!!!');
			}
		}
	}
